...
workingdir = [PALA_data_folder '/PALA_data_InVivoMouseTumor'];
filename = 'PALA_InVivoMouseTumor_';
% workingdir = [PALA_data_folder '/PALA_data_InVivoRatBrain_part1/PALA_data_InVivoRatBrain'];
% filename = 'PALA_InVivoRatBrain';
cd(workingdir)
...
% for hhh = 1:Nbuffers
    ...
    for idx = 1:100
        figure(111); imagesc(log_compression(abs(tmp.IQ(:, :, idx))), [-40, 0])

        % Filtering of IQ to remove clutter (optional)
        IQ_filt = SVDfilter(tmp.IQ,ULM.SVD_cutoff);tmp = [];
        figure(222); imagesc(log_compression(abs(IQ_filt(:, :, idx))), [-40, 0])

        % Temporal filtering
        IQ_filt = filter(but_b,but_a,IQ_filt,[],3); %(optional)
        IQ_filt(~isfinite(IQ_filt))=0;
        figure(333); imagesc(log_compression(abs(IQ_filt(:, :, idx))), [-40, 0])
        pos = ULM_localization2D(abs(IQ_filt(:, :, idx)), ULM);
        hold on;
        plot(pos(:, 3), pos(:, 2), 'r.')
        hold off;
%         pos_all = ULM_localization2D(abs(IQ_filt(:, :, 1)), ULM);    
%         plot(pos_from_all(:, 2), pos_from_all(:, 1), 'ko')
